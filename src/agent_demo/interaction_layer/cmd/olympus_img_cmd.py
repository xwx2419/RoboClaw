...
from agent_demo.common.root_logger import setup_root_logging, table_to_str
import traceback
import logging
...
class Session:
    ...
    async def get_img_forever(self):
        self.running = True
        try:
            while self.running:
                await anyio.sleep(5)  # 每隔x秒取一次图
                await self.agent.add_img_data_to_memory()
        except asyncio.CancelledError:
            logger.info("Img Cancelled.")
        finally:
            self._agent_card.robot_dataloader.shutdown()  # type: ignore
            logger.info("[🚪] robot_dataloader shutdown down")
...
async def input_watcher(queue: asyncio.Queue[str], agent: ImgActAgent):
    running = True
    _agent = agent
    try:
...
async def main():
    udp_config = {
        "udp": {"listen": {"host": "0.0.0.0", "port": 23333}, "send": {"host": "192.168.31.161", "port": 23334}}
    }
    ...
    async with anyio.create_task_group() as tg:
        ...
        # 注册任务：启动 Session，并给它一个 queue
        tg.start_soon(session_inst.run_forever, queue, udp_data)
        await anyio.sleep(0.1)  # 保证Session启动
        # 注册任务：后台每x秒取一次机器人的拼接图
        tg.start_soon(session_inst.get_img_forever)
        # 注册任务：启动命令行输入监听
